...
# Add hollow top for damage slot
    ...
    flag_start_x = reboot_x_offset + reboot_slot_width + Config.space_between_slots * 2 + Config.priority_token_radius - Config.space_between_slots
    ...
    for i in range(6):
        # Create cube
        flag_cube = Part.makeBox(flag_cutout_height,
                                flag_cutout_width,
                                flag_cutout_depth)
    ...
    token_height = Config.priority_token_height * 1.5 + Config.clearance  # Reduced from *2 to *1.5 (25% reduction)
    torus_z = Config.bottom_thickness + Config.card_width - (Config.priority_token_height + Config.clearance) * 2  # Store torus Z
    ...
    priority_token.translate(App.Vector(
        reboot_x_offset + reboot_slot_width + Config.space_between_slots + Config.priority_token_radius,
        y_offset + sphere_radius + reboot_slot_thickness + Config.priority_token_radius/2 + Config.space_between_slots,
        Config.bottom_thickness + Config.card_width - token_height  # Adjusted to keep top at same Z
    ))
    ...
    misc_box_height = inner_box.BoundBox.XLength - (x_offset + slot_height) - Config.space_between_slots * 2
    misc_box_width = inner_box.BoundBox.YLength - (y_offset + upgrade_slot.BoundBox.YLength + slot_spacing * 2) - Config.space_between_slots * 2
    ...
    def exclude_reboot_short_walls(edge):
        ...
        v1 = edge.Vertexes[0]
        v2 = edge.Vertexes[1]
        if are_value(v1.Point.z, v2.Point.z, 61.0) \
    ...
    for i, edge in enumerate(hollow_box.Edges):
        ...
        if (abs(v1.Point.z - target_z) < Config.tolerance and 
            abs(v2.Point.z - target_z) < Config.tolerance):
            
            # Check if edge contains any of our specific coordinates
            edge_has_target_coord = False
            for x in x_coords:
                if (abs(v1.Point.x - x) < Config.tolerance and 
                    abs(v2.Point.x - x) < Config.tolerance):
                    edge_has_target_coord = True
                    break
                    
            for y in y_coords:
                if (abs(v1.Point.y - y) < Config.tolerance and 
                    abs(v2.Point.y - y) < Config.tolerance):
                    edge_has_target_coord = True
                    break
