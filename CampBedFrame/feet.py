...
def fillet_vertical_edges(part, corner_radius):
    for edge in part.Edges:
        if abs(edge.Vertexes[0].Point.x - edge.Vertexes[1].Point.x) < 0.01 and \
        abs(edge.Vertexes[0].Point.y - edge.Vertexes[1].Point.y) < 0.01:
...
def add_grip_cylinders(leg, leg_radius, leg_side_length, leg_thickness):
    ...
    straight_offset = leg_side_length + cyl_radius + (cyl_radius - protrusion) # protrude in the negative direction
    ...
    for x, y in all_positions:
        # Create cylinder
        cyl = Part.makeCylinder(cyl_radius, leg_thickness, 
                               App.Vector(x, y, 0))
        
        # Chamfer top edge
        top_edge = next(edge for edge in cyl.Edges 
...
hollow_cut = make_arc_cut(leg_radius - wall_thickness, leg_radius - 2*(leg_side_length + wall_thickness), leg_thickness, 90)
