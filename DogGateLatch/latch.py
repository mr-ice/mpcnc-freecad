...
hinge_height = (
    max_diameter * 2.25 + hinge_width
)  # offset to center is 1.5, another .75 makes it match the length
...
def create_latch_mechanism(
    width, base_length, height, gap, latch_length, clearance=0.5, thickness=5.0, debug_edges=None
):
    # Create the main lever parts
    lever_cylinder = Part.makeCylinder(total_length / 2, thickness)
    lever_cylinder.translate(
        Base.Vector((total_length / 2 + thickness) / 2, total_length / 2 + thickness, 0)
    )
    ...
    left_hinge_clip.translate(
        Base.Vector(
            0,
            math.sqrt((hinge_height + 50) ** 2 / 2) + left_hinge_supported_length - clip_protruding,
            -math.sqrt((hinge_height + 50) ** 2 / 2),
        )
    )
    ...
    left_cone_hole = Part.makeCone(
        max_diameter / 2 + hinge_clearance, min_diameter / 2 + hinge_clearance, cone_height
    )
    ...
    right_cone_hole.translate(
        Base.Vector(lever_width - hinge_width, hinge_length / 2, -max_diameter * 1.5)
    )
    ...
    indent_depth = 1.75
    indent = Part.makeCylinder(
        indent_diameter / 2, indent_depth, Base.Vector(0, 0, 0), Base.Vector(0, 1, 0)
    )
    ...
    left_wedge.translate(
        Base.Vector(hinge_width, total_length + hinge_length / 2, -latch_wedge_protrusion)
    )
    ...
    transition = Part.makeCylinder(
        transition_radius, thickness, Base.Vector(0, 0, 0), Base.Vector(0, 0, 1), transition_angle
    )
    ...
    transition.rotate(Base.Vector(0, 0, 0), Base.Vector(0, 0, 1), 180)
    transition.translate(
        Base.Vector(transition_radius - hinge_width * 4, total_length - hinge_length / 2, 0)
    )
...
def create_base_mount(width, base_length, height, clearance=0.5, thickness=5.0, max_diameter=10.0):
    ...
    mount = Part.makeBox(width + 2 * thickness, base_length + 2 * thickness, height + 2 * thickness)

    hollow = Part.makeBox(
        width + 2 * clearance, base_length + 2 * clearance, height + 2 * thickness
    )
    ...
    indent_depth = 1.75
    indent = Part.makeCylinder(
        indent_diameter / 2, indent_depth, Base.Vector(0, 0, 0), Base.Vector(0, 0, -1)
    )
    ...
    left_cone.translate(
        Base.Vector(0, base_length / 2, height + 2 * thickness - max_diameter * 1.5)
    )
    ...
    right_cone.translate(
        Base.Vector(
            width + 2 * thickness, base_length / 2, height + 2 * thickness - max_diameter * 1.5
        )
    )
...
def main():
    ...
    latch = create_latch_mechanism(
        width, base_length, base_height, gap, latch_length, debug_edges="vertical"
    ...
    base.translate(
        Base.Vector(base_x_offset, base_y_offset, post_height - base_height - wall_thickness)
    )

    # latch.translate(Base.Vector(- wall_thickness - clearance, 0, post_height + wall_thickness + clearance))
    latch.translate(
        Base.Vector(
            base.BoundBox.XLength / 2 - lever_width / 2 - clearance * 2 - hinge_width,
            0,
