...
def create_hinge_positive():
    ...
    box = Part.makeBox(box_length, REINFORCE_SIZE, THICKNESS/2,
                      App.Vector(0, -REINFORCE_SIZE, -THICKNESS/4))
    
    # Create main cylinder
    cylinder = Part.makeCylinder(HINGE_RADIUS, HINGE_LENGTH, 
                               App.Vector(0, 0, 0), App.Vector(1, 0, 0))
    
    # Create truncated cone at end
    cone_length = HINGE_RADIUS
    top_cone = Part.makeCone(HINGE_RADIUS, HINGE_TOLERANCE, cone_length,
                        App.Vector(HINGE_LENGTH, 0, 0), App.Vector(1, 0, 0))
    bottom_cone = Part.makeCone(HINGE_RADIUS, HINGE_TOLERANCE, cone_length,
                        App.Vector(0, 0, 0), App.Vector(-1, 0, 0))
    # Fuse parts together
...
def create_hinge_negative():
    ...
    top_cone = Part.makeCone(r0, r1, h, 
                            App.Vector(-HINGE_LENGTH - HINGE_TOLERANCE, 0, 0), App.Vector(1, 0, 0))
    bottom_cone = Part.makeCone(r0, r1, h,
                               App.Vector(HINGE_TOLERANCE, 0, 0), App.Vector(-1, 0, 0))
...
def create_sign():
    ...
    panel1.rotate(App.Vector(-HINGE_END_OFFSET - HINGE_TOLERANCE - HINGE_CENTER_OFFSET, 0, 0), 
                 App.Vector(0,0,1), -rotation_angle)
    
    # Fine adjustment for panel1 alignment
    # panel1.translate(App.Vector(1.459, 8.380, 0))
    panel1.translate(App.Vector(.104+ 1.768, 9.420 + HINGE_TOLERANCE, 0))
    obj = doc.addObject("Part::Feature", "Panel1")
    obj.Shape = panel1
    
    # Panel 3 to the right
    panel3.rotate(App.Vector(0,0,0), App.Vector(0,0,1), rotation_angle)
    panel3.translate(App.Vector(BASE_WIDTH + spacing - .1747, 0, 0))
