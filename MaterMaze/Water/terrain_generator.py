...
def create_water_tile(width_squares=2, length_squares=2, thickness_mm=2.25, wave_amplitude=0.5):
    ...
    for i in range(steps + 1):
        ...
        for j in range(steps + 1):
            ...
            random.seed(
                i * 73 + j * 31 + datetime.datetime.now().microsecond
            )  # Prime numbers for less regular patterns
    ...
    try:
        water_surface.buildFromPolesMultsKnots(
            poles, mults_u, mults_v, knots_u, knots_v, False, False, degree, degree
        )
        ...
        for y in range(width_squares + 1):
            ...
            cutter.translate(
                Base.Vector(y * config.square_size, -0.2, 0.6)
            )  # three layers at 0.2mm
            cuts.append(cutter)
            cutterObj = doc.addObject("Part::Feature", "ChannelCutter")
            cutterObj.Shape = cutter
