...
# l
box_chamfer_target_length = sqrt(box_chamfer_target_height**2 + box_chamfer_target_height**2)
...
hole_chamfer_target_height = (
    box_chamfer_target_height.Value
    - sqrt(thickness.Value**2 + thickness.Value**2)
    + thickness.Value
)
...
hole_chamfer_target_length = sqrt(hole_chamfer_target_height**2 + hole_chamfer_target_height**2)
...
doc.Chamfer.ViewObject.LineColor = getattr(
    doc.Box.getLinkedObject(True).ViewObject, "LineColor", doc.Chamfer.ViewObject.LineColor
)
doc.Chamfer.ViewObject.PointColor = getattr(
    doc.Box.getLinkedObject(True).ViewObject, "PointColor", doc.Chamfer.ViewObject.PointColor
)
...
doc.Chamfer001.ViewObject.LineColor = getattr(
    doc.Chamfer001.Box001.getLinkedObject(True).ViewObject,
    "LineColor",
    doc.Chamferr001.ViewObject.LineColor,
)
doc.Chamfer001.ViewObject.PointColor = getattr(
    doc.Chamfer001.Box001.getLinkedObject(True).ViewObject,
    "PointColor",
    doc.Chamfer001.ViewObject.PointColor,
)
Gui.activeDocument().resetEdit()
...
doc.Cut.ViewObject.ShapeColor = getattr(
    doc.Chamfer.getLinkedObject(True).ViewObject, "ShapeColor", doc.Cut.ViewObject.ShapeColor
)
doc.Cut.ViewObject.DisplayMode = getattr(
    doc.Chamfer.getLinkedObject(True).ViewObject, "DisplayMode", doc.Cut.ViewObject.DisplayMode
)
doc.recompute()
