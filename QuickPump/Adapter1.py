...
def cut_slot(shape):
    ...
    path_points = [
        ...
        Base.Vector(
            0, params.BASE_OD / 2 + params.SLOT_LENGTH, params.SLOT_DEPTH
        ),  # Horizontal out
    ]
...
def create_adapter():
    ...
    first_ridge = create_fillet(
        first_ridge, params.FIRST_RIDGE_OD, params.RIDGE_HEIGHT / 2 - precision
    )
    ...
    mid_section.translate(Base.Vector(0, 0, params.BASE_HEIGHT + params.RIDGE_HEIGHT))
    ...
    second_ridge = create_fillet(
        second_ridge, params.SECOND_RIDGE_OD, params.RIDGE_HEIGHT / 2 - precision
    )
    second_ridge.translate(
        Base.Vector(0, 0, params.BASE_HEIGHT + params.RIDGE_HEIGHT + params.MID_HEIGHT)
    )
    top_section = create_tapered_section(
        params.TOP_OD, params.TOP_ID, params.TOP_UPPER_OD, params.TOP_UPPER_ID, params.TOP_HEIGHT
    )
    top_section.translate(
        Base.Vector(
            0,
            0,
            params.BASE_HEIGHT + params.RIDGE_HEIGHT + params.MID_HEIGHT + params.RIDGE_HEIGHT,
        )
    )
    final_shape = (
