...
def get_floor_height_info(voltmeter_top_z=None):
    ...
    # After extra_3
    extra_3_x_start = (voltmeter_x_start + vm_config.EXTRA_BIT_1_WIDTH +
    ...
    for slot in config.COMPARTMENTS:
        ...
        if isinstance(slot[0], list):
            ...
            for i in range(num_compartments):
                ...
                y_pos = (internal_y_start +
                         i * (compartment_y_size +
                              config.INNER_WALL_THICKNESS))
                
                # Get floor height for this X position
                floor_z, floor_height = get_floor_height_at_x(
                    current_x)
...
def get_floor_height_at_x(x_pos) -> tuple[float, float]:
    ...
    available_depth = [
        ...
        config.BASE_HEIGHT - (2 * config.OUTER_WALL_THICKNESS) - vm_config.VOLTMETER_HEIGHT - max( vm_config.EXTRA_BIT_1_HEIGHT, vm_config.EXTRA_BIT_2_HEIGHT, vm_config.EXTRA_BIT_3_HEIGHT ),
    ...
    if x_pos > 2 * config.OUTER_WALL_THICKNESS + vm_config.EXTRA_BIT_1_WIDTH + vm_config.EXTRA_BIT_2_WIDTH + vm_config.EXTRA_BIT_3_WIDTH:
...
def fillet_outer_corners(shape):
    ...
    try:
        ...
        if edges_to_fillet:
            filleted = shape.makeFillet(
                config.CORNER_FILLET_RADIUS,
                edges_to_fillet
            )
            return filleted
