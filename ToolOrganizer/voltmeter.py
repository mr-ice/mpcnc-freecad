...
def create_voltmeter_shape(tolerance=0.0):
    ...
    try:
        ...
        if len(edges_to_fillet) >= 2:
            extra_bit_1 = extra_bit_1.makeFillet(fillet_radius, edges_to_fillet[:2])
...
def create_voltmeter_hole_with_finger_cutout(tolerance=None):
    ...
    extra_bit_3 = Part.makeBox(extra_3_w, extra_3_depth, extra_3_h)
    # Position adjacent to extra_bit_2 (after extra_bit_1 + extra_bit_2)
    extra_bit_3.translate(
        FreeCAD.Vector(
            extra_1_w
            + extra_2_w
            + config.EXTRA_BIT_2_TRANSLATE,  # After extra_bit_1 and extra_bit_2
            0,
            height - tolerance,  # On top of main body
        )
    )
    ...
    try:
        ...
        if len(edges_to_fillet) >= 2:
            extra_bit_3 = extra_bit_3.makeFillet(fillet_radius, edges_to_fillet[:2])
    except Exception as e:
