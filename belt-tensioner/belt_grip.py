...
with open(args.out, "w") as fh:
    ...
    if args.part == "top":
        d.append(
            b.bolt.rotate([0, -90, 0]).translate(
                [b.cap.h + config.boltgrip, o, config.griplength / 2]
            )
        )
        d.append(
            b.bolt.rotate([0, -90, 0]).translate(
        ...
        u.append(
            ops.Cube(
                [config.gripdepth + config.boltgrip, config.beltwidth, config.griplength]
            ).translate([0, -config.beltwidth / 2, 0])
        )
