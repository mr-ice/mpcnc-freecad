...
def create_grommet(diameter, depth, rim_width, thickness, friction_embed_percent=50, cut_width=None, taper_percent=10):
    """
    ...
    if cut_width is not None:
        ...
        center_cyl = Part.makeCylinder(center_cylinder_radius, center_cylinder_height, center_pos, App.Vector(0,0,1))
        
        # Create fillet at the top
        edges_to_fillet = []
        for edge in center_cyl.Edges:
            if abs(edge.CenterOfMass.z - (center_cylinder_height + thickness/2)) < 0.1:  # Top edge
                edges_to_fillet.append(edge)
