...
def semicylinder(l=length, r=radius, h=height):
    ...
    cut.ViewObject.ShapeColor = getattr(
        App.getDocument("Unnamed").getObject("Cylinder").getLinkedObject(True).ViewObject,
        "ShapeColor",
        App.getDocument("Unnamed").getObject("Cut").ViewObject.ShapeColor,
    )
    cut.ViewObject.DisplayMode = getattr(
        App.getDocument("Unnamed").getObject("Cylinder").getLinkedObject(True).ViewObject,
        "DisplayMode",
        App.getDocument("Unnamed").getObject("Cut").ViewObject.DisplayMode,
    )
