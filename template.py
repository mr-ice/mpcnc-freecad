...
def make_plate(doc, x_mm, y_mm, height_mm, pos_x, pos_y, name):
    print(
        f"Making plate {name} at {pos_x}, {pos_y} "
        f"with size {x_mm}x{y_mm}x{height_mm}"
    )
...
def create_keyhole_shape(center_x, center_y, pin_radius, slot_width, slot_length, thickness):
    ...
    cylinder = Part.makeCylinder(
        ...
        App.Vector(center_x, center_y - pin_radius, 0),
        App.Vector(0, 0, 1)
    )
    ...
    slot_box = Part.makeBox(
        ...
        slot_length,
        thickness,
        App.Vector(center_x - slot_half_width, slot_bottom_y, 0)
    )
...
def make_label(
    doc, text, pos_x, pos_y, name, height_mm=1.0, font_size="5.0 mm"
):
    ...
    try:
        ...
        if gui_obj and hasattr(gui_obj, "FontSize"):
            ...
            print(
                f"Set font size to {font_size} for {name} "
                f"(internal: {internal_name})"
            )
        else:
            print(
                f"Warning: Could not find GUI object {internal_name} "
                f"or no FontSize attribute"
            )
    ...
    try:
        ...
        try:
            ...
            if hasattr(Part, "makeShapeString"):
                text_shape = Part.makeShapeString(
                    text, font_name, font_size_mm, 0
                )
...
def extrude_text(doc, label_obj, extrusion_height=1.0):
    ...
    text_shape = create_text_shape(
        text_string, font_name, font_size_val, position
    )
    ...
    for i, size in enumerate(sizes.sizes):
        ...
        for obj_to_cut in objects_to_cut:
            try:
                ...
                print(
                    f"Warning: Could not cut {obj_to_cut.Name} "
                    f"from wrapper: {e}"
                )
        ...
        if max_dimension > config.max_printer_dimension:
            ...
            pin_shape = create_keyhole_shape(
                pin_center_x, pin_y, config.pin_radius, config.slot_width, config.slot_length, config.thickness
            )
            ...
            hole_shape = create_keyhole_shape(
                0, 0, hole_radius, config.slot_width + config.pin_hole_tolerance * 2, hole_slot_length, config.thickness
            )
            hole_shape.rotate(
                App.Vector(0, 0, 0), App.Vector(0, 0, 1), 90
            )
